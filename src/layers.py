...
class IF(nn.Module):
    def __init__(self, T=0, L=8, thresh=8.0, tau=1., gama=1.0):
        ...
        # Add a buffer to store spike counts
        self.register_buffer('spike_count', torch.tensor(0.0))

    def forward(self, x):
        if self.T > 0:
            # Reset spike count at the beginning of a forward pass
            ...
            for t in range(self.T):
                ...
                # Count the number of spikes generated in this timestep
                self.spike_count += torch.sum(spike > 0)
                mem = mem - spike
